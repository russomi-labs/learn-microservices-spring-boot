...
public class ChallengeServiceImpl implements ChallengeService {
	...
	public ChallengeAttempt verifyAttempt(ChallengeAttemptDTO attemptDTO) {
		...
		boolean isCorrect = attemptDTO.getGuess() == attemptDTO.getFactorA()
				* attemptDTO.getFactorB();

		// Builds the domain object. Null id since it'll be generated by the DB.
		ChallengeAttempt checkedAttempt = new ChallengeAttempt(null, user,
				attemptDTO.getFactorA(), attemptDTO.getFactorB(),
				attemptDTO.getGuess(), isCorrect);

		// Stores the attempt
		ChallengeAttempt storedAttempt = attemptRepository.save(checkedAttempt);

		// Sends the attempt to the gamefication and prints response
		boolean status = gamificationServiceClient.sendAttempt(storedAttempt);
		log.info("gamificationServiceClient returned: {}", status);
